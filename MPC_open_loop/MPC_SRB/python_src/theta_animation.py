...
for t, p in folders:
    ...
    x0 = x_data[0]
    times.append(t)
    theta_hist.append(x0[ROLL_COL])
    v_hist.append(x0[PITCH_COL])
    omega_hist.append(x0[YAW_COL])

    # current planned first input u0 (alpha) at time k (applied over next interval)
    u0_alpha_list.append(u_data[0, AL_Z_COL])

    # ---- state predictions: x1..xN plotted at t+DT..t+N*DT ----
    if x_data.shape[0] >= 2:
        theta_pred = x_data[1:, ROLL_COL]
        v_pred     = x_data[1:, PITCH_COL]
        omega_pred = x_data[1:, YAW_COL]
...
# θ
(line_theta,)      = ax_theta.plot([], [], lw=2, label="roll (hist)")
(pred_theta_line,) = ax_theta.plot([], [], lw=2, label="roll (pred)")
(pt_theta,)        = ax_theta.plot([], [], marker='o', linestyle='')
ax_theta.set_ylabel("roll[rad]")
ax_theta.grid(True)
ax_theta.legend()

# v
(line_v,)      = ax_v.plot([], [], lw=2, label="pitch (hist)")
(pred_v_line,) = ax_v.plot([], [], lw=2, label="pitch (pred)")
(pt_v,)        = ax_v.plot([], [], marker='o', linestyle='')
ax_v.set_ylabel("pitch[rad]")
ax_v.grid(True)
ax_v.legend()

# ω
(line_omega,)      = ax_omega.plot([], [], lw=2, label="yaw (hist)")
(pred_omega_line,) = ax_omega.plot([], [], lw=2, label="yaw (pred)")
(pt_omega,)        = ax_omega.plot([], [], marker='o', linestyle='')
ax_omega.set_ylabel("yaw[rad]")
ax_omega.set_xlabel("t[ms]")
ax_omega.grid(True)
ax_omega.legend()

# α
(line_alpha,)      = ax_alpha.plot([], [], lw=2, label="alz (applied hist)")
(pred_alpha_line,) = ax_alpha.plot([], [], lw=2, label="alz (pred u0..uH-1)")
(pt_alpha,)        = ax_alpha.plot([], [], marker='o', linestyle='')
ax_alpha.set_ylabel("alz[m/s^2]")
ax_alpha.set_xlabel("t[ms]")
ax_alpha.grid(True)
ax_alpha.legend()

...
class walkingPlanner {
  ...
  void offline_plan(const double& dt, bool log_plan, const Eigen::Vector3d& pcom){
    ...
    for (int t_step = 0; t_step < N_STEP_; ++t_step){
      ...
      x_ref.col(t_step)(6)  = roll;
      x_ref.col(t_step)(7)  = pitch;
      x_ref.col(t_step)(8)  = yaw;

      x_ref.col(t_step)(9)  = 0.0;
      x_ref.col(t_step)(10) = 0.0;
      x_ref.col(t_step)(11) = omega;

      x_ref.col(t_step)(12) = 0.0;
      x_ref.col(t_step)(13) = v_contact_z;    // v_contact_z;

      x_ref.col(t_step)(14) = 0.0;
      x_ref.col(t_step)(15) = v_contact_z;    // v_contact_z;

      x_ref.col(t_step)(16) = 0;            // jump state
    }


    for (int t_step = 0; t_step < N_STEP_ - 1; ++t_step){
      u_ref.col(t_step)(0) = 0.0;      // ar
      u_ref.col(t_step)(1) = 0.0;      // ar_z

      u_ref.col(t_step)(2) = 0.0;      // al
      u_ref.col(t_step)(3) = 0.0;      // al_z

      u_ref.col(t_step)(4) = 0;         // fr_x
      u_ref.col(t_step)(5) = 0;         // fr_y
      u_ref.col(t_step)(6) = 0; //m*grav/2;  // fr_z

      u_ref.col(t_step)(7) = 0;         // fl_x
      u_ref.col(t_step)(8) = 0;         // fl_y
      u_ref.col(t_step)(9) = 0; //m*grav/2;  // fl_z
    }
